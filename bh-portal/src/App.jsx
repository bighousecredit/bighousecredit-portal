...
// ─── BRAND + HELPERS ────────────────────────────────────────────
// Pure helpers and the brand palette live in src/lib/helpers.js so they
// can be unit tested in isolation (see src/lib/helpers.test.js).
import { BH, daysLeft, monthsOld, usedPct, $$, fdt, uid, aprSt, healthScore } from "./lib/helpers.js";

// ─── AUTH ──────────────────────────────────────────────────────
// Credential map + validation live in src/lib/auth.js (unit tested in
...
function Login({onAuth}){
  ...
  const go=()=>{
    const res=authenticate(email,pw);
    if(!res.ok){setErr(res.error);return;}
    onAuth({role:res.role,cid:res.cid});
  };
